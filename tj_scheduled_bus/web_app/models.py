...
class Enterprise(models.Model):
    ...
    enterprise_type = models.ForeignKey(Status, related_name='enterprise_type_code', null=True, blank=True)      # 企业类型，41-自有车辆企业，42-租赁公司
    ...
    enterprise_status = models.ForeignKey(Status, related_name='enterprise_status_code', null=True, blank=True)      # 站点状态，1-未提交，2-待审核，3-正常，4-审核不通过， 5-冻结
    enterprise_reason = models.CharField(max_length=200, null=True, blank=True)  # 不通过、冻结原因
    user = models.ForeignKey(User, null=True, blank=True)  # 企业所属用户
...
class Road(models.Model):
    ...
    road_area = models.ForeignKey(Area, null=True, blank=True)  # 所属区域


# 方向信息
class Direction(models.Model):
    direction_name = models.CharField(max_length=200, null=True, blank=True)  # 方向名称
    direction_road = models.ForeignKey(Road, null=True, blank=True)  # 所属道路
...
class Station(models.Model):
    ...
    station_status = models.ForeignKey(Status, null=True, blank=True)      # 站点状态，31-运行，32-停运，33-未启动


# 路线信息
class Route(models.Model):
    route_name = models.CharField(max_length=200, null=True, blank=True)                     # 路线名称
    route_station = models.ForeignKey(Station, null=True, blank=True)   # 路线关联站点
    route_user = models.ForeignKey(User, null=True, blank=True)     # 路线所属用户
    route_status = models.IntegerField(default=1, null=True, blank=True)      # 站点状态，1-临时，2-正式


# 路线站点
class RouteStation(models.Model):
    route = models.ForeignKey(Route, null=True, blank=True)   # 路线
    station = models.ForeignKey(Station, null=True, blank=True)   # 站点


# 车辆信息
class Vehicle(models.Model):
    vehicle_number = models.CharField(max_length=50, null=True, blank=True)     # 车牌号
    vehicle_type = models.ForeignKey(Status, related_name='vehicle_type_code', null=True, blank=True)      # 站点状态，21-中型车，22-大型车
    ...
    vehicle_belong = models.ForeignKey(Status, related_name='vehicle_belong_code', null=True, blank=True)      # 车辆所属，11-自有车辆，22-租赁车辆
    vehicle_status = models.ForeignKey(Status, related_name='vehicle_status_code', null=True, blank=True)  # 车辆状态，1-未审核，2-待审核，3-审核通过，4-未通过，5-冻结
    vehicle_reason = models.CharField(max_length=200, null=True, blank=True)  # 审核不通过/冻结原因
    vehicle_user = models.ForeignKey(User, null=True, blank=True)  # 车辆所属用户


# 通行证信息
class Permission(models.Model):
    permission_vehicle = models.ForeignKey(Vehicle, null=True, blank=True)  # 车辆
    permission_route = models.CharField(max_length=200, null=True, blank=True)  # 路线名称
    permission_user = models.ForeignKey(User, null=True, blank=True)  # 通行证用户
    expiration_date = models.DateTimeField(null=True, blank=True)     # 期限
